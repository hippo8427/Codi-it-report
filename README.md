...
4. 노션 프로젝트 계획서의 표 형식의 관리 뷰를 이용해 프로젝트 진행도와 팀원들의 작업 내용, 기간, 상태를 명확하게 파악할 수 있었습니다.   
...
4. 마이그레이션 파일 관리 체계화
   ...
   현재는 각자 생성한 마이그레이션 파일 충돌을 방지하기 위해 .gitignore로 관리에서 제외하였습니다.  
   하지만 마이그레이션 파일을 공유하는 것이 안정적인 협업과 배포에 매우 유리하다는걸 알게 되었습니다.  
   추후에는 마이그레이션을 한 명 또는 전담 프로세스에서 관리하여, 충돌을 방지할 계획입니다.
